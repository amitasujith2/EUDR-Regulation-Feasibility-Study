...
// Clip the datasets to the area of interest
var shapefileAssetClipped = shapefileAsset.filterBounds(areaOfInterest);
var providedDatasetClipped = providedDataset.filterBounds(areaOfInterest);

// Print the feature collection to the console for inspection
print('Shapefile Feature Collection:', shapefileAssetClipped);
print('Protected Areas Feature Collection:', providedDatasetClipped);
...
// Clip the alert datasets to the area of interest
RADD = RADD.clip(areaOfInterest);
GLAD_L = GLAD_L.clip(areaOfInterest);
GLAD_S2 = GLAD_S2.clip(areaOfInterest);

// Forest baseline layers
Map.addLayer(RADD_forest_baseline.clip(areaOfInterest), {palette:['black'], opacity: 0.3},'RADD forest baseline',false);
Map.addLayer(GLAD_L_forest_baseline.clip(areaOfInterest), {palette:['black'], opacity: 0.3},'GLAD_L forest baseline',false);
Map.addLayer(GLAD_S2_forest_baseline.clip(areaOfInterest), {palette:['black'], opacity: 0.3},'GLAD_S2 forest baseline',false);
...
Map.addLayer(ruleset1.clip(areaOfInterest), alertVisParams, 'Integrated alert (Ruleset-1)', true);
Map.addLayer(ruleset2.clip(areaOfInterest), alertVisParams, 'Integrated alert (Ruleset-2)', false);
...
Map.addLayer(ruleset1_YYDOYdateFormat.clip(areaOfInterest), { bands: 'Date', min: 19000, max: 25000, palette: ['black', 'white'] }, 'Integrated alert (Ruleset-1) (YYDOY)', false);

var ruleset1_FractionalYear = alertFunctions.convertDatesToFractionalYear(ruleset1);
Map.addLayer(ruleset1_FractionalYear.clip(areaOfInterest), { bands: 'Date', min: 2019, max: 2025, palette: ['black', 'white'] }, 'Integrated alert (Ruleset-1) (Fractional year)', false);


// Pixel-based integration with considering spatial neighborhood
var ruleset1_buffered = alertFunctions.integrateAlerts({
    alerts: [RADD, GLAD_L, GLAD_S2],
    ruleset: 'r1',
...
Map.addLayer(ruleset1_buffered.clip(areaOfInterest), alertVisParams, 'Integrated alert (Ruleset-1) [100m buffer]', false);
